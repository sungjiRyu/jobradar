...
#### 4-1. 캐시 적용 대상 및 선정 근거
* **적용 API:** `/api/stats/today`, `/api/stats/location`, `/api/stats/tech-stack`
* **선정 근거:**
  * **연산 비용 (Heavy-Read):** 지역별, 기술스택별 공고 집계 쿼리는 다중 테이블 `JOIN`과 `GROUP BY`를 동반하여 DB 리소스 소모가 매우 큽니다.
  * **데이터 특성 (High-Cache-Hit):** 채용 공고 통계는 초 단위의 엄격한 실시간성이 요구되지 않으며, 변경 주기에 비해 조회 빈도가 압도적으로 높기 때문에 캐시 효율이 극대화될 수 있다고 판단했습니다.

#### 4-2. 데이터 정합성 유지 (TTL 및 갱신)
캐시 데이터와 원본 DB 간의 불일치(Stale Data)를 최소화하고, 동시에 서버 부하를 방어하기 위해 다음과 같은 전략을 취했습니다.

* **TTL(Time-To-Live) 설정:** 실시간성과 DB 부하의 트레이드오프를 고려하여 모든 통계 캐시에 의 만료 시간을 설정했습니다.
* **이벤트 기반 무효화 (Eviction):** 신규 채용 공고가 등록되거나 상태가 변경(마감 등)되는 핵심 트랜잭션이 발생할 때, Spring의 `@CacheEvict` 를 활용하여 관련된 통계 캐시만을 선택적으로 삭제해 정합성을 유지했습니다.

#### 4-3. 최종 성능 개선 결과 (인덱스 + 캐싱 결합)
