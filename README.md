...
### 1. 채용공고 통합 검색
- 사람인·잡코리아 개발 직군 공고를 한 곳에서 검색 (06/03 기준 30,000건, 매일 업데이트)
...
애플리케이션과 데이터베이스의 타임존 설정이 일치하지 않았습니다. 한국 표준시(KST)는 UTC보다 9시간 빠르기 때문에, 날짜를 기준으로 집계하는 쿼리와 스케줄러의 실행 시간이 예상과 다르게 동작했습니다.

**해결**

애플리케이션과 데이터베이스에서 사용할 타임존을 명시적으로 설정하고, 스케줄러가 Asia/Seoul 기준으로 실행되도록 구성했습니다.
