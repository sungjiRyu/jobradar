...
운영 환경 배포 후 신규 공고가 0건으로 조회되고, 의도한 시각과 다른 시간에 스케줄러가 실행되는 문제가 발생했습니다.

**원인**

EC2, RDS, 애플리케이션의 타임존 기준이 명확히 일치하지 않았습니다.
본 서비스는 공고 마감일, 오늘 등록된 공고, 통계 집계, 스케줄러 실행 시간을 한국 시간 기준으로 처리해야 합니다. 하지만 서버 또는 데이터베이스가 UTC 기준으로 동작하면 KST와 9시간 차이가 발생하여 `LocalDate.now()`, `CURRENT_DATE`, 스케줄러 cron 실행 시각이 예상과 다르게 해석될 수 있습니다.


**해결**

- 가장 먼저 서버의 timezon 설정을 직접 바꾸는 방법을 생각했습니다.
- 하지만 나중에 scal-out 등으로 다중 인스턴스 환경이 된다면 인스턴스마다 서버의 timezone을 일일히 바꿔줘야 됩니다.
- 따라서 timezone 설정은 애플리케이션 단에서 관리하는게 맞다는 생각이 들었습니다.

애플리케이션 단에서 명시적으로 timezone 설정을 해서  날짜 기반 조회, 통계 집계, 마감 공고 처리, 크롤링 스케줄이 모두 한국 시간 기준으로 동작하도록 정리했습니다.
<br>
